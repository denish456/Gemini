...
let plusButton, plusDropdown; // Also declare these globally here
...
function renderRecentChats() {
    ...
    data.chats.forEach(chat => {
        ...
        li.classList.add('flex', 'items-center', 'gap-4', 'cursor-pointer', 'overflow-hidden', 'text-ellipsis', 'relative', 'group'); // Added group for kebab opacity
        ...
        span.classList.add('flex-grow', 'overflow-hidden', 'text-ellipsis'); // Ensures text truncation
        ...
        kebabMenuWrapper.className = 'kebab-menu-wrapper absolute right-2 top-1/2 -translate-y-1/2 flex items-center';
        kebabMenuWrapper.innerHTML = `
                    <button class="kebab-button material-icons-outlined rounded-full hover:bg-[var(--bg-hover)] w-8 h-8 flex items-center justify-center">:</button>
                    <div class="kebab-dropdown hidden absolute right-0 top-full mt-1 w-32 rounded-lg shadow-lg z-20 text-sm py-1"
                        style="background-color: var(--dropdown-bg); color: var(--dropdown-text); border: 1px solid var(--dropdown-border);">
                        <button class="block w-full text-left px-3 py-1 hover:bg-[var(--dropdown-hover)]">Pin</button>
                        <button class="block w-full text-left px-3 py-1 hover:bg-[var(--dropdown-hover)]">Rename</button>
                        <button class="block w-full text-left px-3 py-1 hover:bg-[var(--dropdown-hover)]">Delete</button>
                    </div>
                `;
        li.appendChild(kebabMenuWrapper);

        // Event listener for kebab button
        ...
        kebabButton.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent li click and document click from immediately closing
        ...
        kebabDropdown.querySelector('button:nth-child(1)').addEventListener('click', () => {
            // Pin functionality - Placeholder
            console.log('Pin clicked for chat:', chat.id);
            kebabDropdown.classList.add('hidden');
        });

        kebabDropdown.querySelector('button:nth-child(2)').addEventListener('click', () => {
            // Rename functionality - Using prompt for now, replace with custom modal later
            ...
            if (newTitle && newTitle.trim() !== "") {
                ...
                if (chatToRename) {
                    ...
                    setChatData(data);
                    renderRecentChats(); // Re-render to update title in sidebar
                }
            }
            kebabDropdown.classList.add('hidden');
        });

        kebabDropdown.querySelector('button:nth-child(3)').addEventListener('click', () => {
            // Delete functionality - Using confirm for now, replace with custom modal later
            if (confirm('Are you sure you want to delete this chat?')) {
                let data = getChatData();
                data.chats = data.chats.filter(c => c.id !== chat.id);
                if (data.activeChatId === chat.id) {
                    data.activeChatId = null; // Clear active chat if deleted
                }
                setChatData(data);
                renderRecentChats(); // Re-render to update list
                if (currentChatId === chat.id || data.chats.length === 0) { // If current chat was deleted, or no chats left
                    createNewChat();
                }
            }
            kebabDropdown.classList.add('hidden');
        ...
        li.addEventListener('click', (e) => {
            // Ensure clicking the kebab menu or its dropdown doesn't trigger loading the chat
